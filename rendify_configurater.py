...
from RendifyLauncher import (
    ...
    run_roblox_installer, is_process_running, kill_roblox,
)
...
ENGINE_FLAGS = {
    "Graphics": {
        "DFIntTextureCompositorQualityLevel": {
            "type": "int",
            "default": 3,
            "min": 0,
            "max": 3,
            "label": "Texture Compositor Quality"
        },
        ...
        "FIntDebugForceMSAASamples": {
            ...
            "min": 1,
            "max": 16,
            "label": "Force MSAA (Multi-Sample Anti-Aliasing) Samples"
        },
    ...
    "Rendering": {
        ...
        "FFlagDebugGraphicsPreferD3D10": {
            "type": "bool",
            "default": False,
            "label": "Prefer Direct3D 10"
        },
        "FFlagDebugGraphicsDisableD3D11": {
            "type": "bool",
            "default": False,
            "label": "Disable Direct3D 11"
        },
        ...
        "DFFlagDebugRenderForceTechnologyFuture": {
            "type": "bool",
            "default": True,
            "label": "Force Future Rendering"
        },
        "DFFlagDebugRenderForceTechnologyVoxel": {
            "type": "bool",
            "default": False,
            "label": "Force Voxel Rendering"
        },
    },
    "Level of Detail": {
        ...
        "DFIntCSGLevelOfDetailSwitchingDistanceL12": {
            ...
            "min": 0,
            "max": 2000,
            "label": "LOD Switching Distance (L1-L2)"
        },
        "DFIntCSGLevelOfDetailSwitchingDistanceL23": {
            ...
            "min": 0,
            "max": 2000,
            "label": "LOD Switching Distance (L2-L3)"
        },
        "DFIntCSGLevelOfDetailSwitchingDistanceL34": {
            ...
            "min": 0,
            "max": 2000,
            "label": "LOD Switching Distance (L3-L4)"
        },
    },
    "Performance": {
        "DFIntPerformanceControlManualGpuMemoryLimit": {
            "type": "int",
            "default": 128,
            "min": 0,
            "max": 8192,
        ...
        "DFIntPerformanceControlManualCpuBudget": {
            "type": "int",
            "default": 1,
            "min": 0,
            "max": 10,
        ...
        "FFlagDebugGraphicsDisableTextureQualityReduction": {
            "type": "bool",
            "default": False,
            "label": "Disable Texture Quality Reduction"
        },
    },
    "Experimental": {
        "FFlagDebugDisableTelemetry": {
            "type": "bool",
            "default": True,
        ...
        "FFlagDebugDisableTelemetryEphemeralStat": {
            "type": "bool",
            "default": True,
            "label": "Disable Ephemeral Telemetry"
        },
        "FFlagDisableNewIGMinimization3": {
            "type": "bool",
            "default": True,
            "label": "Disable New IG Minimization"
        },
        "DFFlagDebugForceAppCapResizableWindow": {
            "type": "bool",
            "default": False,
            "label": "Resizable Window Cap"
        },
        "FFlagHandleAltEnterFullscreenManually": {
            "type": "bool",
            "default": False,
            "label": "Manual Alt+Enter Fullscreen"
        },
    },
}
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        icon_ico = resource_path("RendifyLogo.ico")
        icon_png = resource_path("RendifyLogo.png")
        chosen_icon = icon_ico if os.path.exists(icon_ico) else icon_png
        if os.path.exists(chosen_icon):
            self.after(0, lambda: self._set_icons(chosen_icon))
    ...
    def _set_icons(self, icon_path):
        ...
        try:
            if icon_path.lower().endswith(".png") and os.path.exists(icon_path):
    ...
    def _build_engine_ui(self, parent, data):
        for category, flags in ENGINE_FLAGS.items():
            ...
            for name, meta in flags.items():
                ...
                ctk.CTkLabel(
                    row, text=label_text, width=200, anchor="w",
                    font=ctk.CTkFont(size=11)
                ).pack(side="left")

                ctk.CTkLabel(
                    row, text=f"({name})", width=220, anchor="w",
                ...
                if typ == "bool":
                    ...
                    if choices:
                        ...
                        values = [value for value, _ in choice_items]
                        labels = [label for _, label in choice_items]

                        def _find_default_label(default_value):
                            for value, label in choice_items:
                                if value == default_value:
                                    return label
                            return labels[0] if labels else str(default_value)

                        var = tk.StringVar(
                            value=_find_default_label(default))
    ...
    def _engine_collect(self):
        ...
        for name, info in self._engine_widgets.items():
            ...
            if typ == "bool":
                ...
                var, values, labels = info[1], info[2], info[3]
                label = var.get()
                if label in labels:
    ...
    def _launch_with_status(self, pid, config_name, game_name):
        ...
        def run():
            ...
            if proc:
                while True:
                    if is_process_running("RobloxPlayerInstaller.exe"):
                        time.sleep(0.5)
                        continue
                    time.sleep(2.5)
                    if not is_process_running("RobloxPlayerInstaller.exe"):
                        break
            self.launcher_exe = find_roblox_player_launcher()
