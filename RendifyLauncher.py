...
def find_roblox_content_textures_path():
    version_dir = find_roblox_latest_version()
    if not version_dir:
        return None
    path = os.path.join(version_dir, "content", "textures")
    return path if os.path.isdir(path) else None
...
def apply_cursors(theme_name):
    ...
    dst_keyboard = find_roblox_cursors_path()
    dst_textures = find_roblox_content_textures_path()
    if not dst_keyboard or not dst_textures:
        return
    ...
    for f in os.listdir(src):
        ...
        s = os.path.join(src, f)
        if os.path.isfile(s):
            shutil.copy2(s, os.path.join(dst_keyboard, f))
        elif os.path.isdir(s):
            d = os.path.join(dst_keyboard, f)
            if os.path.isdir(d):
                shutil.copytree(s, d, dirs_exist_ok=True)
            else:
                shutil.copytree(s, d)

    # Roblox stores the shift-lock cursor in the root content textures folder,
    # not in the KeyboardMouse subfolder, so ensure that override is copied too.
    shiftlock_src = os.path.join(src, "MouseLockedCursor.png")
    if os.path.isfile(shiftlock_src):
        shutil.copy2(
            shiftlock_src,
            os.path.join(dst_textures, "MouseLockedCursor.png")
        )
